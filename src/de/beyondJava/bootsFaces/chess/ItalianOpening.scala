package de.beyondJava.bootsFaces.chess

import scala.concurrent.duration._

import io.gatling.core.Predef._
import io.gatling.http.Predef._
import io.gatling.jdbc.Predef._

class ItalianOpening extends Simulation {

	val httpProtocol = http
		.baseURL("http://www.angularfaces.net")
		.inferHtmlResources(BlackList(""".*\.js""", """.*\.css""", """.*\.gif""", """.*\.jpeg""", """.*\.jpg""", """.*\.ico""", """.*\.woff""", """.*\.(t|o)tf""", """.*\.png""", """.*\.js.jsf.*"""), WhiteList())
		.acceptHeader("text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8")
		.acceptEncodingHeader("gzip, deflate")
		.acceptLanguageHeader("de,es;q=0.8,en-US;q=0.6,en;q=0.4,fr-FR;q=0.2,fr;q=0.2,de-DE;q=0.2")
		.doNotTrackHeader("1")
		.userAgentHeader("Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_4) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2661.94 Safari/537.36")

	val headers_0 = Map(
		"Accept" -> "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8",
		"Accept-Encoding" -> "gzip, deflate, sdch",
		"AlexaToolbar-ALX_NS_PH" -> "AlexaToolbar/alx-4.0",
		"Upgrade-Insecure-Requests" -> "1")

	val headers_1 = Map(
		"Accept" -> "text/css,*/*;q=0.1",
		"Accept-Encoding" -> "gzip, deflate, sdch",
		"AlexaToolbar-ALX_NS_PH" -> "AlexaToolbar/alx-4.0")

	val headers_3 = Map(
		"AlexaToolbar-ALX_NS_PH" -> "AlexaToolbar/alx-4.0",
		"Content-Type" -> "application/x-www-form-urlencoded; charset=UTF-8",
		"Faces-Request" -> "partial/ajax",
		"Origin" -> "http://www.angularfaces.net")

    val uri1 = "http://www.angularfaces.net/BootsFacesChess-1.0.0-SNAPSHOT"

	val scn = scenario("ItalianOpening")
		.exec(http("request_0")
			.get("/BootsFacesChess-1.0.0-SNAPSHOT/")
			.headers(headers_0)
			.resources(http("request_1")
			.get(uri1 + "/javax.faces.resource/theme.css.jsf?ln=primefaces-aristo")
			.headers(headers_1)
			.check(status.is(304)),
            http("request_2")
			.get(uri1 + "/javax.faces.resource/css/theme.css.jsf?ln=bsf")
			.headers(headers_1)
			.check(status.is(304))))
		.pause(16)
		.exec(http("request_3")
			.post("/BootsFacesChess-1.0.0-SNAPSHOT/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "5")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:rows:6:fields:4:j_id_n", "form:rows:6:fields:4:j_id_n")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:rows:6:fields:4:j_id_n")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "@all")
			.formParam("javax.faces.partial.render", "form messages")
			.formParam("form", "form")
			.resources(http("request_4")
			.post(uri1 + "/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "4")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:opponent", "form:opponent")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:opponent")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "form:opponent")
			.formParam("javax.faces.partial.render", "form")
			.formParam("form", "form"),
            http("request_5")
			.post(uri1 + "/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "4")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:rows:4:fields:4:j_id_n", "form:rows:4:fields:4:j_id_n")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:rows:4:fields:4:j_id_n")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "@all")
			.formParam("javax.faces.partial.render", "form messages")
			.formParam("form", "form"),
            http("request_6")
			.post(uri1 + "/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "4")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:opponent", "form:opponent")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:opponent")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "form:opponent")
			.formParam("javax.faces.partial.render", "form")
			.formParam("form", "form")))
		.pause(9)
		.exec(http("request_7")
			.post("/BootsFacesChess-1.0.0-SNAPSHOT/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "4")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:rows:7:fields:6:j_id_n", "form:rows:7:fields:6:j_id_n")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:rows:7:fields:6:j_id_n")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "@all")
			.formParam("javax.faces.partial.render", "form messages")
			.formParam("form", "form")
			.resources(http("request_8")
			.post(uri1 + "/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "4")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:opponent", "form:opponent")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:opponent")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "form:opponent")
			.formParam("javax.faces.partial.render", "form")
			.formParam("form", "form"),
            http("request_9")
			.post(uri1 + "/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "4")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:rows:5:fields:5:j_id_n", "form:rows:5:fields:5:j_id_n")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:rows:5:fields:5:j_id_n")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "@all")
			.formParam("javax.faces.partial.render", "form messages")
			.formParam("form", "form"),
            http("request_10")
			.post(uri1 + "/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "4")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:opponent", "form:opponent")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:opponent")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "form:opponent")
			.formParam("javax.faces.partial.render", "form")
			.formParam("form", "form")))
		.pause(3)
		.exec(http("request_11")
			.post("/BootsFacesChess-1.0.0-SNAPSHOT/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "4")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:rows:7:fields:5:j_id_n", "form:rows:7:fields:5:j_id_n")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:rows:7:fields:5:j_id_n")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "@all")
			.formParam("javax.faces.partial.render", "form messages")
			.formParam("form", "form")
			.resources(http("request_12")
			.post(uri1 + "/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "4")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:opponent", "form:opponent")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:opponent")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "form:opponent")
			.formParam("javax.faces.partial.render", "form")
			.formParam("form", "form")))
		.pause(1)
		.exec(http("request_13")
			.post("/BootsFacesChess-1.0.0-SNAPSHOT/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "4")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:rows:4:fields:2:j_id_n", "form:rows:4:fields:2:j_id_n")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:rows:4:fields:2:j_id_n")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "@all")
			.formParam("javax.faces.partial.render", "form messages")
			.formParam("form", "form")
			.resources(http("request_14")
			.post(uri1 + "/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "4")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:opponent", "form:opponent")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:opponent")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "form:opponent")
			.formParam("javax.faces.partial.render", "form")
			.formParam("form", "form")))
		.pause(13)
		.exec(http("request_15")
			.post("/BootsFacesChess-1.0.0-SNAPSHOT/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "5")
			.formParam("input_form:j_id_10", "7")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:rows:7:fields:4:j_id_n", "form:rows:7:fields:4:j_id_n")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:rows:7:fields:4:j_id_n")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "@all")
			.formParam("javax.faces.partial.render", "form messages")
			.formParam("form", "form")
			.resources(http("request_16")
			.post(uri1 + "/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "4")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:opponent", "form:opponent")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:opponent")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "form:opponent")
			.formParam("javax.faces.partial.render", "form")
			.formParam("form", "form"),
            http("request_17")
			.post(uri1 + "/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "4")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:rows:7:fields:6:j_id_n", "form:rows:7:fields:6:j_id_n")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:rows:7:fields:6:j_id_n")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "@all")
			.formParam("javax.faces.partial.render", "form messages")
			.formParam("form", "form"),
            http("request_18")
			.post(uri1 + "/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "4")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:opponent", "form:opponent")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:opponent")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "form:opponent")
			.formParam("javax.faces.partial.render", "form")
			.formParam("form", "form")))
		.pause(12)
		.exec(http("request_19")
			.post("/BootsFacesChess-1.0.0-SNAPSHOT/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "1")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:rows:6:fields:3:j_id_n", "form:rows:6:fields:3:j_id_n")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:rows:6:fields:3:j_id_n")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "@all")
			.formParam("javax.faces.partial.render", "form messages")
			.formParam("form", "form")
			.resources(http("request_20")
			.post(uri1 + "/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "1")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:opponent", "form:opponent")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:opponent")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "form:opponent")
			.formParam("javax.faces.partial.render", "form")
			.formParam("form", "form"),
            http("request_21")
			.post(uri1 + "/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "1")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:rows:5:fields:3:j_id_n", "form:rows:5:fields:3:j_id_n")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:rows:5:fields:3:j_id_n")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "@all")
			.formParam("javax.faces.partial.render", "form messages")
			.formParam("form", "form"),
            http("request_22")
			.post(uri1 + "/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "1")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:opponent", "form:opponent")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:opponent")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "form:opponent")
			.formParam("javax.faces.partial.render", "form")
			.formParam("form", "form")))
		.pause(3)
		.exec(http("request_23")
			.post("/BootsFacesChess-1.0.0-SNAPSHOT/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "1")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:rows:7:fields:2:j_id_n", "form:rows:7:fields:2:j_id_n")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:rows:7:fields:2:j_id_n")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "@all")
			.formParam("javax.faces.partial.render", "form messages")
			.formParam("form", "form")
			.resources(http("request_24")
			.post(uri1 + "/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "1")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:opponent", "form:opponent")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:opponent")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "form:opponent")
			.formParam("javax.faces.partial.render", "form")
			.formParam("form", "form"),
            http("request_25")
			.post(uri1 + "/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "1")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:rows:3:fields:6:j_id_n", "form:rows:3:fields:6:j_id_n")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:rows:3:fields:6:j_id_n")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "@all")
			.formParam("javax.faces.partial.render", "form messages")
			.formParam("form", "form"),
            http("request_26")
			.post(uri1 + "/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "1")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:opponent", "form:opponent")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:opponent")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "form:opponent")
			.formParam("javax.faces.partial.render", "form")
			.formParam("form", "form")))
		.pause(9)
		.exec(http("request_27")
			.post("/BootsFacesChess-1.0.0-SNAPSHOT/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "1")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:rows:7:fields:1:j_id_n", "form:rows:7:fields:1:j_id_n")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:rows:7:fields:1:j_id_n")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "@all")
			.formParam("javax.faces.partial.render", "form messages")
			.formParam("form", "form")
			.resources(http("request_28")
			.post(uri1 + "/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "1")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:opponent", "form:opponent")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:opponent")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "form:opponent")
			.formParam("javax.faces.partial.render", "form")
			.formParam("form", "form"),
            http("request_29")
			.post(uri1 + "/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "1")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:rows:5:fields:2:j_id_n", "form:rows:5:fields:2:j_id_n")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:rows:5:fields:2:j_id_n")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "@all")
			.formParam("javax.faces.partial.render", "form messages")
			.formParam("form", "form"),
            http("request_30")
			.post(uri1 + "/index.jsf")
			.headers(headers_3)
			.formParam("form_board_collapsed", "false")
			.formParam("form_j_id_r_collapsed", "false")
			.formParam("input_form:j_id_y", "1")
			.formParam("input_form:j_id_10", "5")
			.formParam("form:j_id_12_helper", "oxff")
			.formParam("form_j_id_v_collapsed", "false")
			.formParam("form_j_id_13_collapsed", "false")
			.formParam("form_SUBMIT", "1")
			.formParam("javax.faces.ViewState", "9AenpS2MCG9q362lW1Fzvj3aEc/FjbBYXw6O1RjR9XYNnf2F")
			.formParam("form:opponent", "form:opponent")
			.formParam("javax.faces.partial.event", "click")
			.formParam("javax.faces.source", "form:opponent")
			.formParam("javax.faces.partial.ajax", "true")
			.formParam("javax.faces.partial.execute", "form:opponent")
			.formParam("javax.faces.partial.render", "form")
			.formParam("form", "form")))

	setUp(scn.inject(atOnceUsers(1))).protocols(httpProtocol)
}